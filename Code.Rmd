...
```{r}
naive_lin_reg <- lm(outcome_var ~ treatment_var + factor(S1) + factor(S2) + factor(S3) + factor(S3) + factor(S4) + factor(I1) + factor(I2) + factor(I1):factor(I2) + factor(PARTY7) + factor(REL1) + PPAGE + factor(PPEDUCAT)+ factor(PPETHM) + factor(PARTY7):factor(PPEDUCAT) + factor(PPGENDER) + factor(PPGENDER):factor(PPWORK) + factor(PPHHHEAD) + PPHHSIZE + factor(PPHOUSE) + factor(PPINCIMP) + factor(PPMARIT) + factor(PPMSACAT) + factor(PPREG4) + factor(PPRENT) + PPT01 + PPT25 + PPT612 + PPT1317 + PPT18OV + factor(PPWORK) + factor(PPNET) + factor(PPNET):factor(PPMSACAT), data=data5, model=TRUE, x=TRUE, y=TRUE, qr=TRUE)
naive_lin_reg
```
